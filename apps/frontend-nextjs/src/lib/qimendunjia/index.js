...
class Qimen {
  ...
  pan_sky() {
    ...
    let rotate = {
      阳: Config.clockwise_eightgua,
      阴: Config.anti_clockwise_eightgua,
  ...
  pan_star() {
    ...
    let star_reorder = {
      阳: Config.new_list(Config.star_r, starting_star),
      阴: Config.new_list(Config.star_r.slice().reverse(), starting_star),
    }[this.qimen_ju_name()[0]];
    let gong_reorder = [];
    if (starting_gong == "中") {
      gong_reorder = Config.new_list(rotate, "坤");
  ...
  pan_door() {
    ...
    let door_reorder = {
      阳: Config.new_list(Config.door_r, starting_door),
      阴: Config.new_list(Config.door_r.slice().reverse(), starting_door),
    }[this.qimen_ju_name()[0]];
    let gong_reorder = [];
    if (starting_gong == "中") {
      gong_reorder = Config.new_list(rotate, "坤");
  ...
  // 暗干
  pan_angan() {
    let gans = Config.yingyang_order['阳遁'];
    let starting_gong = this.zhifu_n_zhishi()["值使門宮"][1];
    let tianpan = this.pan_earth()[0];
    let rotate = {
      阳: Config.eight_gua,
      阴: Config.eight_gua.slice().reverse(),
    }[this.qimen_ju_name()[0]];
    let gong_reorder = [];
    let angan_reorder = [];
  ...
  crane_god() {
    ...
    let crane_list = ["巽", "离", "坤", "兑", "乾", "坎", "天", "艮", "震"];
    let newc_list = [];
  ...
  gpan() {
    ...
    let start_gong_d = { 冬至: "艮离坎坤震巽", 夏至: "坤离巽坤离兑" };
    let yy_dun = { 冬至: "阳遁", 夏至: "阴遁" };
    ...
    let start_gong = start_gong_d[jieqi_key] || "艮离坎坤震巽"; // 默认值

    let yy = yy_dun[jieqi_key] || "阳遁"; // 默认阳遁
    ...
    let g = [];
    let f = {
      阴遁: Config.anti_clockwise_eightgua,
      阳遁: Config.clockwise_eightgua,
    };
  ...
  dizhi_pan() {
    ...
    let eight_gua2=['坎','艮','震','巽','离','坤','兑','乾']
    let dizhi_pan = {
      '坎': ['子'],
      '艮': ['丑', '寅'],
      '震': ['卯'],
