...
# 云丁未奇门遁甲React版移动APP构建脚本
echo "🚀 开始构建云丁未奇门遁甲React版移动APP..."
...
update_ios_version() {
    ...
    if [[ $current_name =~ (.+)\ V([0-9]+)\.([0-9]+)\.([0-9]+)$ ]]; then
        ...
        new_name="$base_name V1.0.1"
        echo "  iOS添加初始版本号: V1.0.1"
    fi
...
if [ $node_version -lt 18 ]; then
    echo "❌ 需要Node.js 18+版本，当前版本: $(node -v)"
    echo "请运行: nvm use 18"
    exit 1
fi
...
# 构建Web应用 - 移动端专用构建
echo "🔨 构建React应用（移动端配置）..."
echo "📱 设置移动端API配置: http://101.201.148.8:3001"
...
echo ""
echo "📋 快速命令参考："
echo "构建React: npm run build"
echo "同步平台: npx cap sync"  
echo "打开Android: npx cap open android"
echo "打开iOS: npx cap open ios"
echo "实时调试Android: npx cap run android --livereload"
echo "实时调试iOS: npx cap run ios --livereload"
echo ""
echo "📱 版本管理信息："
echo "• 每次构建时会自动递增版本号"
echo "• iOS版本从 V1.0.1 开始递增"
echo "• Android版本从 V1.0.1 开始递增"
echo "• 版本号会显示在应用名称中"
echo ""
echo "📝 React项目特色："
echo "• 使用现代React技术栈"
echo "• Redux Toolkit状态管理"
echo "• 完整的移动端适配"
echo "• 与Vue版本功能一致" 