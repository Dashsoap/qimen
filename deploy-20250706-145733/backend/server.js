...
app.get('/api/checkin/status', authenticateToken, async (req, res) => {
  try {
    ...
    // 查找今日签到记录
    const todayCheckin = await prisma.pointsRecord.findFirst({
      where: {
        userPoints: {
          userId: req.user.id
        },
    ...
    // 获取连续签到天数
    const continuousStreak = await getCheckinStreak(req.user.id);
    ...
    res.json({
      ...
      data: {
        ...
        todayBonus: getCheckinBonus(continuousStreak + 1)
      }
    });
...
app.post('/api/checkin', authenticateToken, async (req, res) => {
  try {
    ...
    const updatedPoints = await prisma.userPoints.update({
      where: { userId: req.user.id },
      data: {
        balance: userPoints.balance + bonus,
        totalEarned: userPoints.totalEarned + bonus,
        pointsRecords: {
          create: {
            amount: bonus,
            type: 'earned',
            description: `每日签到奖励 (连续${newStreak}天)`
          }
        }
      },
      include: {
    ...
    res.json({
      ...
      data: {
        ...
        nextDayBonus: getCheckinBonus(newStreak + 1)
      }
    });
...
app.get('/api/checkin/history', authenticateToken, async (req, res) => {
  try {
    ...
    const checkinHistory = await prisma.pointsRecord.findMany({
      where: {
        userPoints: {
          userId: req.user.id
        },
    ...
    const total = await prisma.pointsRecord.count({
      where: {
        userPoints: {
          userId: req.user.id
        },
...
async function getCheckinStreak(userId) {
  try {
    const checkinRecords = await prisma.pointsRecord.findMany({
      where: {
        userPoints: {
          userId: userId
        },
...
// 辅助函数：计算签到奖励
function getCheckinBonus(streak) {
  if (streak === 1) return 10;      // 第1天：10积分
  if (streak <= 3) return 15;       // 第2-3天：15积分
  if (streak <= 7) return 20;       // 第4-7天：20积分  
  if (streak <= 15) return 30;      // 第8-15天：30积分
  if (streak <= 30) return 50;      // 第16-30天：50积分
  return 100;                       // 第31天及以上：100积分
}
